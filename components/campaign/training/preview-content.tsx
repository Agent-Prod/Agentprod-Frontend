...
function PreviewContent() {
  ...
  const fetchFollowUpData = async () => {
    try {
      ...
      const response1 = await axiosInstance.post(endpoint, {
        user_id: user?.id,
        follow_up_number: 1,
        campaign_id: params.campaignId,
        previous_emails: [
          {
            subject: channel === "Linkedin" ? "" : autoGeneratedSubject,
      ...
      const response2 = await axiosInstance.post(endpoint, {
        user_id: user?.id,
        follow_up_number: 2,
        campaign_id: params.campaignId,
        previous_emails: [
          ...
          {
            subject: channel === "Linkedin" ? "" : response1.data.subject,
            body: channel === "Linkedin" ? response1.data.message : response1.data.body
          },
        ],
        channel: channel,
      });
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center space-x-3">
          ...
          <h3 className="text-sm font-medium">
            {channel === "Linkedin" ? `LinkedIn Follow-up ${number}` : `Follow-up Email ${number}`}
          </h3>
        </div>
      ...
      <ResizablePanel defaultSize={70}>
        <div className="flex justify-center p-6">
          <div className="flex-col w-full max-w-3xl space-y-6">
            ...
            <Card className="p-6">
              {channel !== "Linkedin" && <EmailHeader />}

              <div className="space-y-4">
                <div className="flex items-center justify-between">
                  <h3 className="text-sm font-medium text-muted-foreground">
                    {channel !== "Linkedin" ? "Main Email Content" : "LinkedIn Message"}
                  </h3>
                </div>
                <Textarea
                  className={cn(
            ...
            {campaignType !== "Nurturing" && (
              <div className="space-y-4">
                <h3 className="text-sm font-medium text-muted-foreground">
                  {channel === "Linkedin" ? "LinkedIn Follow-up Sequence" : "Follow-up Sequence"}
                </h3>
                <FollowUpSection
                  number={1}
                  loading={load1}
                  content={autoGeneratedFollowUp}
                />
                <FollowUpSection
                  number={2}
                  loading={load2}
                  content={autoGeneratedFollowUpTwo}
                />
              </div>
            )}
          </div>
