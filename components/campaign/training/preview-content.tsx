...
function PreviewContent() {
  ...
  const newPreview = async () => {
    ...
    try {
      ...
      if (channel === "omni") {
        ...
        setInviteMessage(response?.linkedin_message?.invite_message || "");
        setMessageAfterInvite(response?.linkedin_message?.message_after_invite || "");
        setLinkedinFollowUp(response?.linkedin_message?.follow_up_bodies || []);
        if (response?.email?.follow_up_bodies) {
        ...
        setLinkedinFollowUp(response?.email?.follow_up_bodies || []);
        setMessageAfterInvite(response?.email?.message_after_invite || "");
        setLinkedinComment(response?.linkedin_comment || "");
      ...
      if (response?.linkedin_information) {
      ...
      toast.error("Failed to generate new preview");
    } finally {
  ...
  const hasContent = (str: string | undefined | null): boolean => {
    return !!str && str.trim().length > 0;
  };

  // Helper function to check if array has content
  const hasArrayContent = (arr: any[]): boolean => {
    return Array.isArray(arr) && arr.length > 0 && arr.some(item => hasContent(item));
  };
          ...
          <div className="flex-col w-full max-w-3xl space-y-6">
            ...
            {(channel === "mail" || channel === "omni") && (
              <>
                ...
                {campaignType !== "Nurturing" && hasArrayContent(followUps) && (
                  <div className="space-y-4">
                    <h3 className="text-sm font-medium text-muted-foreground">
            ...
            {(channel !== "mail") && hasContent(inviteMessage) && (
              <Card className="p-6">
                <div className="space-y-4">
                  <div className="flex items-center justify-between">
                    <h3 className="text-sm font-medium text-muted-foreground">
            ...
            {(channel !== "mail") && hasContent(messageAfterInvite) && (
              <Card className="p-6">
                <div className="space-y-4">
                  <div className="flex items-center justify-between">
                    <h3 className="text-sm font-medium text-muted-foreground">
                      LinkedIn Message After Invite
                    </h3>
                  </div>
                  <Textarea
                    className={cn(
            ...
            {(channel === "Linkedin" || channel === "omni") && (
              hasContent(channel === "omni" ? linkedinMessage : autoGeneratedBody) && (
                <Card className="p-6">
                  <div className="space-y-4">
                    <div className="flex items-center justify-between">
                      <h3 className="text-sm font-medium text-muted-foreground">
            ...
            {(channel === "Linkedin" || channel === "omni") &&
              ...
              hasArrayContent(linkedinFollowUp) && (
                <div className="space-y-4">
                  <h3 className="text-sm font-medium text-muted-foreground">
                    LinkedIn Follow-up Sequence
                  </h3>
                  {linkedinFollowUp.map((followUp: string, index: number) => (
                    hasContent(followUp) && (
                      <FollowUpSection
                        key={index}
                        number={index + 1}
            ...
            {(channel === "Linkedin" || channel === "omni") && hasContent(linkedinComment) && (
              <Card className="p-6">
                <div className="space-y-4">
                  <div className="flex items-center justify-between">
                    <h3 className="text-sm font-medium text-muted-foreground">
